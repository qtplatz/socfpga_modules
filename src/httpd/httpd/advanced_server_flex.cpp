...
handle_request( beast::string_view doc_root, http::request<Body, http::basic_fields<Allocator>> &&req, Send &&send )
{
    ...
    ADDEBUG() << "request_handler: method=" << req.method() << ", uri=" << req.target();
...
template <class Derived> class websocket_session {
    ...
    template <class Body, class Allocator> void do_accept( http::request<Body, http::basic_fields<Allocator>> req )
    {
        ...
        derived().ws().async_accept(
            req, beast::bind_front_handler( &websocket_session::on_accept, derived().shared_from_this() ) );
    }
    ...
    void do_read()
    {
        ...
        derived().ws().async_read(
            buffer_, beast::bind_front_handler( &websocket_session::on_read, derived().shared_from_this() ) );
    }
    ...
    void on_read( beast::error_code ec, std::size_t bytes_transferred )
    {
        ...
        derived().ws().async_write(
            buffer_.data(), beast::bind_front_handler( &websocket_session::on_write, derived().shared_from_this() ) );
    }
...
void
make_websocket_session( beast::tcp_stream stream, http::request<Body, http::basic_fields<Allocator>> req )
{
...
template <class Body, class Allocator>
void
make_websocket_session( beast::ssl_stream<beast::tcp_stream> stream,
  ...
  public:
    ...
    void do_read()
    {
        ...
        http::async_read( derived().stream(), buffer_, *parser_,
                          beast::bind_front_handler( &http_session::on_read, derived().shared_from_this() ) );
    }
...
class ssl_http_session
  ...
  public:
    ...
    ssl_http_session( beast::tcp_stream &&stream, ssl::context &ctx, beast::flat_buffer &&buffer,
                      std::shared_ptr<std::string const> const &doc_root )
...
main( int argc, char *argv[] )
{
    ...
    if ( argc != 5 ) {
        std::cerr << "Usage: advanced-server-flex <address> <port> <doc_root> "
                     "<threads>\n"
                  << "Example:\n"
